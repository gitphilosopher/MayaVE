...
- **Timer alert routing** — the alert now speaks through the `Speaker` injected by `Router` under `run_interruptible`, waits up to 60 s for a free turn, and the expired timer is removed from `_timers` before the alert.
...
### Issues
- **Blocking calls on the event loop** — [Confirmed] `IntentEngine.classify`, `SQLiteVectorStore.search/has_records`, `expression_library.save_recipe`, the `system_info` CPU branch (`psutil.cpu_percent(interval=1)`), the screenshot branch (`pyautogui`) and the Kokoro pipeline rebuild run synchronously on the loop, against the executor rule.
- **Kokoro rebuild blocks loop and resets wrong pipeline** — [Confirmed] `Speaker._synthesise_guarded` rebuilds its pipeline synchronously after a timeout, and `_run_kokoro`'s timeout branch resets the llm_service pipeline even when Speaker's was the stuck one.
- **Timer alert not serialized** — [Confirmed] An alert is not serialized with the queue worker, so a non-name command spoken during it still queues and speaks over it, and it shares the single `_audio_done_event` with any concurrent speech.
- **Reminder and timer parsing gaps** — [Confirmed] "remind me to X" without a duration asks for one, numerals as words ("one hour") are not parsed, and the label lookahead is prefix-based so labels starting a/an/me/the/for/set/min/sec/hour (e.g. "apple timer") are dropped.
- **Residual audio_done stall** — [Confirmed] A stop landing just before `wait_for_audio_done` clears the shared Event still times out after 30 s (tiny window).
- **Skill-exception line stored in history** — [Confirmed] The router's "Sorry senpai, I ran into a problem with that." is still stored as an assistant turn because it isn't flagged `_no_history`.
- **Degenerate context topic and entity data** — [Confirmed] `intent["target"]` is almost always the full utterance, so `_infer_topic`, `_extract_entities` and `_resolve_reference` return the current utterance, making `TOPIC`, `ENTITIES` and `LIKELY REFERRING TO` largely degenerate, topic-scoped dedup rarely match and the empty-target prompt in `google_search` rarely fire.
- **Open website unknown sites** — [Confirmed] Sites outside the 8-entry `_SITES` map (amazon, linkedin and stackoverflow are in TRAINING_DATA) open an invalid `https://amazon`-style URL.
...
- **Kokoro concurrency and lazy init** — [Potential] Filler and first phrase synthesize concurrently on one pipeline (plus Speaker/timer overlap), `_get_kokoro` lazy init is unlocked, a stuck native call cannot be killed (daemon thread orphaned), and Ollama/Kokoro/embeddings may contend for GPU if `tts.device` leaves `"cpu"`.
- **HF offline flag import order** — [Potential] `HF_HUB_OFFLINE` is set after `from kokoro import KPipeline` in `llm_service.py` (and `core/speaker.py` imports kokoro first), so it may not take effect.
