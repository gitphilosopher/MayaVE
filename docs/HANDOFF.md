...
1. `os.makedirs(config.log_dir)` before logging setup. `ws_task = asyncio.create_task(ws_server.serve())` (reference kept; failure logged via `_on_ws_done`); construct `Speaker()`, `Transcriber()`, `Processor(speaker)` (→ `IntentEngine()` load/auto-train — *synchronous, blocks loop*; `ConversationManager`; `Router(speaker)`, which also injects the speaker into `timer.py` via `set_timer_speaker`).
...
|---|---|---|
| FSM | `core/state.py` `StateManager`, singleton `state` | States SLEEPING/IDLE/LISTENING/PROCESSING/SPEAKING/INTERRUPTED; `run_interruptible`, `can_interrupt()`, `interrupt()`, stop callback. |
...
| WS | `services/ws_server.py` `MayaWebSocketServer` (`ws_server`) | Broadcast + `audio_done` gate + interrupt handler; remembers last broadcast state (`_last_state`, default `"idle"`) and replays it to each new client in `_handler`. |
...
**Queue/processor:** `QueueManager` single serial worker; `priority` field unused. `Processor.handle`: PROCESSING + WS state/transcript → `ConversationManager.add_user` → classify → `context_manager.observe_user_turn` → dispatch → if response ≠ `ALREADY_SPOKEN`: `_strip_tags(response)` → tag-free text goes to `add_assistant` (skipped when the skill/LLM set `intent["_no_history"]`, i.e. `query()` error strings) and to the transcript broadcast; then `speaker.speak` with the original tagged text (`greet` intent → `on_audio_start` broadcasts `wave`; a skill-set `intent["action"]` (from `perform_action`) → `on_audio_start` broadcasts that animation). Always ends with WS `idle` + baseline behavior; exceptions → IDLE. State sequence per command: `listening → processing → speaking → idle` (skill/LLM paths set FSM IDLE themselves).
...
**Barge-in (`core/state.py`, `main.py`, `ws_server.py`, `avatar.js`)** — `interrupt()` runs when `can_interrupt()`: SPEAKING, or PROCESSING with a live registered speech task (an LLM turn, including its filler and the wait before the first phrase). Skill turns register no task → not interruptible. Flow: state→INTERRUPTED, stop callback (`sd.stop()` + `broadcast_stop_audio`, which also sets `_audio_done_event`), cancel `_current_task`, state→LISTENING. Tasks are registered via `run_interruptible` by `llm_service.query()` and the timer alert (`timer._alert`); **`Speaker.speak()` from processor/main is never wrapped**, so a barge-in there is stopped only by the stop callback (audio halts, no task cancel). After a voice barge-in the interrupting utterance is queued, so the FSM proceeds LISTENING → PROCESSING normally. A barge-in during an LLM turn leaves history with only the phrases whose playback started (§4).
...
**Blocking-on-loop calls (violates project rule "blocking → executor"):** `IntentEngine.classify`; `SQLiteVectorStore.search/has_records`; `expression_library.save_recipe` (file write); `system_info` CPU branch (`psutil.cpu_percent(interval=1)`); screenshot branch (`pyautogui.screenshot`); `Speaker._synthesise_guarded` pipeline rebuild after a synthesis timeout (`_build_kokoro_pipeline`, model load). (`power._run` correctly uses the executor.)
...
- **Streaming (`_ollama_streamer`):** tokens → `_next_boundary(buffer)` (split at `.!?,;—` followed by whitespace/buffer-end, or after 12 words; comma before the vocative `config.user_name` is skipped; waits when next word not yet streamed; a `.` at the very end of the buffer after an alphanumeric waits for more tokens so "3.14"/"google.com" aren't split) → `_emit` → `_parse_expression` (`*action*` extracted first; then consecutive leading `[..]` tags; `_ANY_BRACKET_RE` strips other brackets) → `synth_q` item `(text, expression, actions, is_final, attitude, intensity, continuation)`. `continuation` = the phrase follows an earlier non-final phrase of the same sentence. Expression/attitude/intensity carry forward across phrases of one sentence until `is_final`. `*...*` spans are removed entirely (so markdown emphasis words are dropped, not just unstyled).
- **Workers:** `_synth_worker` (serial; `_enhance_prosody(text, expr, is_final, continuation)` → `_run_kokoro(_synthesise_blocking)`; puts `(audio, expression, actions, attitude, intensity, phrase)` on `play_q`), `_play_worker` (serial; per phrase: `broadcast_animation`(actions) → `broadcast_behavior(compose(...))` → state `speaking` → `_spoken_phrases.append(phrase)` → `broadcast_audio` → `wait_for_audio_done` → baseline behavior). At `_DONE`: `_reply_finished[0]=True`, `state.set(IDLE)` + WS idle.
- **Filler:** `_should_play_filler` (intent ∈ {general_query, unknown}; model source lacks "fallback/keyword/guard"; not `_NO_FILLER_RE`; ≥3 words) → `asyncio.gather(_play_filler(), _stream_and_speak())`; `_filler_done` event gates only the first real phrase. `query()` sets `_filler_done` in a `finally` so a cancelled filler never leaves the gate closed.
- **History:** `query()` clears `_spoken_phrases`/`_reply_finished` at start. After the reply: finished reply (`_reply_finished`) → `_last_response[0]` (full clean text); interrupted → `" ".join(_spoken_phrases)` (phrases whose playback started; a phrase cut mid-way is recorded whole). Then `_conv.add_assistant`, transcript broadcast, `context_manager.record_assistant_turn`. Empty → nothing recorded (the user turn stays in history without a reply). `query()` error strings (`ConnectError`/timeout/exception) set `intent["_no_history"]=True` and are spoken by `Processor` but not stored.
...
- **Compaction:** `Memory.on_evict` → `_on_memory_evict`; every 10 evicted turns → one `conversation_summary` record: `"Earlier discussion touched on: "` + alphabetically-first 15 keywords (task kept in `_bg_tasks`).
...
- **Two independent `KPipeline`s:** `Speaker._pipeline` (skills/greeting/wake/sleep lines/timer alerts) and `llm_service._kokoro_pipeline` (LLM stream, filler), both built via `_build_kokoro_pipeline`. Both warmed at startup. With `device="cpu"` the duplication is RAM only; with `cuda`/`auto` it is VRAM too.
- **Synthesis:** all Kokoro calls via `llm_service._run_kokoro` (new **daemon thread** per call, 15 s timeout, `_reset_kokoro_pipeline()` on timeout → returns None). `Speaker._synthesise_guarded` distinguishes "no audio produced" (`_NO_AUDIO`, no rebuild) from a timeout (`None` → rebuilds its own pipeline). Output float32 24 kHz → `_numpy_to_wav` (executor) → base64 in JSON.
- **Prosody (`_enhance_prosody(text, expression, is_final, continuation)`):** `_elongation_re_sub` → `_fix_caps` (+`_CAPS_WHITELIST`) → partial chunk: strip trailing punct only; final: `_expand_short_exclamation` (**skipped when `continuation=True`**, so "Well, yes." keeps "yes.") → per-expression punctuation/fragmentation (`_fragment_for_energy` caps at 4 fragments). `EXPRESSION_SPEED` (0.84–1.13) applied **only** in `llm_service._synthesise_blocking`, not in `Speaker._synthesise`.
- **Playback:** one WAV per phrase; browser `speakFromBytes` (WebAudio `AudioBufferSourceNode`), `source.onended` → `_onAudioDone` → WS `{"type":"audio_done"}` → `ws_server._audio_done_event`. `speakFromBytes` also sends `audio_done` when `vrm` isn't loaded or decoding fails (unless a stop intervened), and drops audio whose decode finished after a `stop_audio` (`_audioGen`). `wait_for_audio_done(timeout=30)` **returns False immediately when no client is connected**; otherwise it **clears the single shared Event on entry**, warns and continues on timeout. `broadcast_stop_audio` sets the Event so a barge-in doesn't stall a pending wait. `output="local"` uses `sounddevice`; `"both"` double-plays with the avatar (must stay `"avatar"` when frontend runs).
...
- **Barge-in audio:** `stopCurrentAudio()` bumps `_audioGen`, sets `onended=null` (no `audio_done` is sent after a stop) and stops lip-sync.
...
- **Known problems:** see §11 (wave-vs-speech sync — wave fires in `on_audio_start` *after* synthesis).
...
**Expression Lab (`frontend/expression-lab.html` + `js/expression-lab.js`):** dev-server-only page (not a Vite build input). Slider edits persist immediately to `localStorage["maya.expressionLab.recipes"]` (schema v1) and set a `_dirty` flag; only an edited (`_dirty`) combination is saved on switch/Export, so merely browsing a combination does **not** pin its generated default. Import merges an `expressions.json` (and clears `_dirty`); **Export writes only the localStorage set** (drops backend-generated/never-imported entries) — Import first. Its `composeDefault` mirrors `core/expression_library.py` (duplicated intentionally). Attitude list is wider than the backend's.
...
- **WS (`websocket.js`):** reconnect every 2 s; handles `audio, state, behavior, stop_audio, transcript(no-op), animation(wave|nod|giggle|sigh|shrug|wink)`; sends `audio_done` only. No `interrupt` sender exists in the frontend. URL hard-coded `ws://localhost:8765`. `handleState` forwards every value to `setAvatarState` (fidget gate, resets/starts `_idleSince`) and, for `listening`, sets `surprised` 0.3 via `setExpression`; the next `behavior` packet overwrites it. The backend sends the last known `state` right after connect, so `_currentBackendState` is set at connect rather than staying `null`.
...
- **VRMA (`playVrmaAnimation(name)`):** `_VRMA_ASSETS` maps animation names to `.vrma` files (filenames often differ from names — read the map; all mapped files exist as LFS pointers); URL-cached loads; `createVRMAnimationClip` → keep only `.quaternion` (minus shoulders via `_PROTECTED_BONE_KEYS`) and `.weight` tracks; fade in/out; claims bones via `animation-controller` (FIDGET tier for `_FIDGET_VRMA_NAMES`, else ACTION); cleanup `setTimeout` does `action.stop(); mixer.update(0)` **only if `_activeMixers` doesn't already hold a different mixer for that name** (a halted run's stale timer must not touch a replay); a non-fidget play halts active VRMA fidgets first and also calls `_headTiltStop?.()`. `wink`, `headTilt`, `shoulderRoll` are procedural (not VRMA); `headTilt` keeps a `_headTiltStop` handle that restores the neck and releases its claim (used by the action halt and by natural completion). `shoulderRoll` isn't halted: shoulders are protected bones no `.vrma` writes.
...
| Timer (`utilities/timer.py`) | `set_timer`, `cancel_timer`, `timer_status`, **and `set_reminder`** (router maps it here) | named/numbered asyncio timers; `_parse_duration` uses one pattern per unit (`hours?\|hrs?`, `minutes?\|mins?`, `seconds?\|secs?`; digits only — "one hour" is not parsed); cancel/status word checks are skipped for `set_reminder`; `_countdown` removes an expired timer from `_timers` (`_forget`, only its own entry) **before** the alert; `_extract_reminder` captures "remind me to X" text (`_REMINDER_RE`, duration stripped, ≤120 chars). `_alert` waits (≤60 s) until `state.is_busy()` is false, then `state.run_interruptible(_speaker.speak(msg))` via the Speaker injected by `Router` (`set_speaker`): "Time's up, senpai! Reminder: X." or "…Your {name} is done." | Alert waits for a free turn because `run_interruptible` holds one `_current_task` (firing mid-turn would overwrite it). Not serialized with the queue worker — a non-name command spoken during the alert still queues and speaks over it; shares the single `_audio_done_event`. "remind me to X" without a duration → "How long should I set the timer for". Label regex's lookahead is prefix-based (labels starting a/an/me/the/for/set/min/sec/hour dropped). |
...
- `ws_server` stores `_last_state` (updated on every `broadcast_state`, even with no clients; default `"idle"`) and `_handler` sends it to each new client → frontend `_currentBackendState` is no longer `null` until the first broadcast. Applied in the latest pass (an earlier revision had documented it without the code).
...
- `set_reminder` vs `set_timer` "nondeterministic skill": **not a bug** — router maps both to `timer`; only `reminder.py` is dead code.
- Duplicate dead `_elongation_re_sub` removed (`llm_service.py` keeps only the non-recursive definition; applied in the latest pass).
...
- **Unreferenced tasks (was #11):** `ws_task` kept + `_on_ws_done`; `ContextManager._bg_tasks`.
- **`ws_server._broadcast` set mutation (was #12):** iterates a copy of `_clients`.
- **Listener stale utterance after sleep (was #13):** `_process_frame` clears the in-progress utterance while SLEEPING.
- **Barge-in history (was #7):** interrupted turns record only `_spoken_phrases`; finished turns record the full reply (`_reply_finished`).
- **Barge-in during PROCESSING/filler (was #8):** `can_interrupt()` allows PROCESSING with a live registered task; `on_speech` uses it for `was_speaking`; `_filler_done` set in a `finally`.
- **30 s `audio_done` stalls (was #2, mostly):** frontend sends `audio_done` when `vrm` missing/decode fails; `broadcast_stop_audio` sets the Event; `_audioGen` drops audio whose decode finished after a stop (was #15).
- **Timer alert (was #1) and reminder text (was #18, part):** alert speaks via injected `Speaker` under `run_interruptible`, waits ≤60 s for a free turn; reminder text captured and spoken; expired timer removed before the alert.
- **`_expand_short_exclamation` mid-sentence (was #9):** skipped for `continuation` chunks (final chunk of an already-split sentence).
- **Stale fidget timeouts / headTilt fights actions (was #14):** cleanup timer guarded by mixer identity; `headTilt` cancellable via `_headTiltStop`, halted when an ACTION animation starts.
- **Expression Lab pinning (was #16):** only `_dirty` (edited) combinations are persisted on switch/Export.
- **Query error strings in history and tags in history/transcript (was #17, part):** `query()` error replies set `intent["_no_history"]`; `Processor` stores/broadcasts tag-stripped text.

### Confirmed (deterministic from source; not runtime-tested) — still open
1. **Timer alert residual:** not serialized with the queue worker (a non-name command spoken during the alert still queues and speaks over it) and shares the single `_audio_done_event` with any concurrent speech.
2. **`audio_done` residual:** a stop landing just before `wait_for_audio_done` is entered (which clears the Event) still times out at 30 s; tiny window.
...
6. **Kokoro rebuild residual:** the pipeline rebuild after a timeout still runs synchronously on the event loop; `_run_kokoro`'s timeout branch resets the *llm_service* pipeline even when Speaker's was the stuck one.
17. **Skill-exception string in history:** `Router.dispatch`'s "[sad] Sorry senpai, I ran into a problem with that." is still stored as an assistant turn (not flagged `_no_history`).
18. **Reminder gaps (timer skill):** "remind me to X" without a duration → "How long should I set the timer for"; numerals as words ("one hour timer", in TRAINING_DATA) are not parsed; the label regex's negative lookahead is prefix-based, so labels starting with a/an/me/the/for/set/min/sec/hour (e.g. "apple timer") are dropped.
...
- **Interrupted LLM turn:** the user turn stays in history with no assistant reply (or a partial one); a barge-in before the first phrase records nothing for the assistant.
- **Skill-path barge-in:** `Speaker.speak()` isn't wrapped in `run_interruptible`, so a barge-in there stops audio (stop callback) but doesn't cancel the task; skill turns in PROCESSING aren't interruptible.
...
- **State replay on connect:** if the last recorded state is `listening`, a late-connecting frontend briefly shows the surprised-0.3 face until the next `behavior` packet; `docs/architecture.md` §10 does not yet list the on-connect `state` message.
...
- `audio_done` is sent from `source.onended` (and from `speakFromBytes` for the no-vrm/decode-failure cases); `stopCurrentAudio` nulls `onended` and bumps `_audioGen` deliberately (a stray `audio_done` after a stop could release a later wait early). `wait_for_audio_done` clears a single shared Event — keep speech serialized.
...
- `query()` reads/clears module-level `_last_response` and resets `_spoken_phrases`/`_reply_finished` at the start of each turn; `_filler_done` starts **set** and is re-set in `query()`'s `finally`; `run_interruptible` swallows `CancelledError`.
...
- `interrupt()` is allowed when `can_interrupt()` (SPEAKING, or PROCESSING with a live task); `main.on_speech` must snapshot `state.can_interrupt()` (not `is_speaking()`) before STT.
...
- `Router.__init__` must call `set_timer_speaker(speaker)` (timer alerts depend on it).

**Contracts**
- WS server→client: `audio{data}`, `state{value: listening|processing|speaking|idle}` (the last known value is also sent once to each new client on connect), `behavior{primary,secondary,intensity,attitude,gaze,actions,recipe}`, `stop_audio`, `transcript{text,role}`, `animation{name}`; client→server: `audio_done`, `interrupt`. `expression` message is legacy/unused and the frontend doesn't handle it.
- `broadcast_stop_audio` must keep setting `_audio_done_event`. `broadcast_state` must keep writing `_last_state` even when no clients are connected, and `_handler` must keep replaying it to each new client (the fidget gate depends on it).
- Skills return `"[tag] text"`; LLM path returns `ALREADY_SPOKEN`. `Speaker._strip_tags` only strips `\[\w+\]` — other brackets (e.g. `[2026-06-15 12:00]`) reach TTS, which is why `notepad.py` strips timestamps itself; LLM text relies on `_ANY_BRACKET_RE`. `Processor` stores/broadcasts `_strip_tags` output, and skips history when `intent["_no_history"]` is set (set by `query()` error branches).
- Queue tuples: `synth_q` = `(text, expression, actions, is_final, attitude, intensity, continuation)`; `play_q` = `((audio, samplerate), expression, actions, attitude, intensity, phrase)`. `_enhance_prosody(text, expression, is_final, continuation)`; `Speaker.speak` calls it with defaults.
...
- `expressions.json` is written by the backend and by Lab Export (flat schema; viseme keys excluded). Import before Export; keep a backup (a corrupt file now blocks backend writes rather than being wiped, but Lab Export overwrites the file you save over).
...
- `headTilt` must go through `_headTiltStop` for both natural end and halting (restores `origZ`, releases the claim).
...
Mic/VAD/STT pipeline; serial queue; intent ensemble + auto-retrain; Ollama streaming with phrase splitting, tag parsing, filler gate, `keep_alive` + turn diagnostics; Kokoro synth path with timeout/reset (CPU by default); WS protocol (`listening` broadcast; last-state replay on connect); behavior engine + recipe generation/persistence (write-protected against unreadable files); mood logic; recent-window + semantic memory pipeline (structure); VRMA player/fidget scheduler; lip-sync; datetime, clipboard, media, weather, open_website (listed sites), google_search, lock_screen (Windows, immediate), power (shutdown/restart with confirmation, Windows), notepad, screenshot (word or intent), timers' duration parsing, timers/reminders with spoken reminder text and interruptible alert, `perform_action` skill animation, LLM `*action*` animations via the LLM playback path, voice sleep/wake state handling, barge-in on LLM turns (playback, filler, pre-first-phrase) with played-phrases-only history.

### Present in code, with known limitations
Barge-in (skill/greeting `Speaker.speak` path: audio stops but task isn't cancelled; skill turns not interruptible); sleep/wake (state holds and wake word is active; frontend never sees sleep; in-flight command can overwrite SLEEPING); timer alert (not serialized with the queue worker, shares `audio_done` event); context state quality (§5); expression library (22/35 entries Lab-only by decision; naming split); gaze (no screen observer); open_app (no name mapping); Expression Lab (dev only); listening/FSM edge cases (§11 Potential).

### Known broken
None confirmed beyond the open items in §11 (§11 Confirmed); `reminder.py` is unused dead code.

### Unverified
VRM morph/expression names and VRMA track contents (LFS pointers); whether recipe morphs persist past `expressionManager.update()`; Ollama model availability and actual keep-alive behavior (check COLD logs / `ollama ps`); STT accuracy/latency; GPU placement and VRAM contention (Kokoro is CPU by default); Kokoro thread-safety; audio echo behavior; production (`vite build`) packaging; actual `npm run dev` Electron launch; runtime behavior of all Batch 1–4 and Batch A/B fixes (incl. `shutdown.exe` invocation) and all audit findings.
...
- Does a frontend connecting late get an initial `state` message right after connect (log `[Maya WS] Connected`, then `state` in devtools; fidgets start after the calm period)? Start the backend, let it go idle, then open the frontend.
- Does "can you giggle/wink/nod" play the animation in sync with the reply? Does "set a timer for 5 minutes" fire at 300 s? Does "remind me to drink water in 15 minutes" speak "Time's up… Reminder: drink water"? Does saying "Maya" during the alert cut it off, and does an alert firing mid-reply wait for it to finish? Re-setting a named timer: is the new one still cancellable? Does the avatar resume fidgeting after a timer alert?
- Barge-in: call her name (a) mid-reply, (b) during the filler, (c) before the first phrase. Does she stop, and does history/transcript contain only the phrases actually played (log line "LLM turn interrupted…" when none)? Does a barge-in during a skill/greeting line stop audio without a 30 s stall?
- Does an Ollama-down error line get spoken but not appear in later LLM context? Do skill replies show up in history without `[tag]` prefixes?
- Does "Well, yes." stay "Well, yes." (not "Oh yes, absolutely!") while a bare "Yes!" still expands?
- Trigger an ACTION animation (e.g. "can you nod") right after a `headTilt`/VRMA fidget: does the neck return to rest afterwards, and does a replayed fidget finish cleanly?
- Expression Lab: browse several combinations without moving a slider, Export — do untouched ones stay out of the file?
...
- Ask "what is pi" style questions: is "3.14" spoken as one number?
...
- Does starting the backend with no frontend connected proceed without a 30 s greeting stall? Does the WS server stay up for long sessions?
...
**Top issues to know (remaining):** timer alert not serialized with the queue worker; Router skill-exception string stored in history; reminder gaps (no-duration reminders, numeral words, label lookahead); degenerate context topic/entity data; residual Kokoro rebuild-on-loop; skill-path barge-in only stops audio; `docs/architecture.md` §10 lacks the on-connect `state` message. Decisions already made: extra `expressions.json` entries stay Lab-only; `lock_screen` immediate; unused config fields kept but marked; interrupted LLM turns record only played phrases; barge-in cancels PROCESSING/filler turns; reminder text kept and alert routed via Speaker/interruptible. Batch 1–4 and Batch A/B fixes are applied but not runtime-tested.

**Fragile:** Kokoro concurrency/timeouts/rebuild; the single `_current_task` in `state`; fidget/animation ownership; `expressions.json` dual writers; closed vocabularies duplicated across files; one-shot power confirmation vs echo.

**Do not break:** §12 (queue order, per-phrase WS order, `audio_done` semantics, `Speaker.speak()` SLEEPING/IDLE contract, `on_speech` LISTENING/`can_interrupt` rules, `resolve_pending` before routing, queue tuple shapes, `_no_history` contract, vocab sync, mood hook placement, `_run_kokoro`, chat `keep_alive`, TRAINING_DATA auto-retrain, `.quaternion/.weight` filtering, idempotent wake, `timer.execute` reminder bypass, timer alert waiting for a free turn, `_last_state` write/replay).
