...
Tasks are registered via `run_interruptible` by `llm_service.query()`, the timer alert (`timer._alert`), `Processor.handle()`'s skill-response `Speaker.speak()` calls, and `main.py`'s startup greeting / wake-up line / sleep goodbye line (Batch 4 — "Skill-path barge-in"). A skill's own blocking dispatch work before it starts speaking (e.g. `weather.py`'s HTTP call) still registers no task and is not interruptible until it reaches its own `Speaker.speak()` call.
...
|---|---|---|
| Recent window | `brain/memory.py` `Memory` singleton `memory` (max 50 entries); wrapped by 3 `ConversationManager` instances (processor, `llm_service._conv`, ContextManager), all sharing `memory`. Methods: `add_user`, `add_assistant`, and a history getter — `brain/memory.py`'s `Memory.get_history(last_n)` is the low-level reader against the raw window; `brain/conversation.py`'s `ConversationManager.get_context(last_n)` is the thin public wrapper skill/LLM callers actually use, and simply calls `memory.get_history(last_n)`. The LLM gets the last 6. `on_evict` fires just before the oldest entry is dropped | In-process only (lost on restart) |
...
- **Config:** `config.llm`: `model="llama3.2"`, `base_url=http://localhost:11434`, `max_tokens=150` → `num_predict`, `temperature=0.7`. `LLMConfig` currently defines only these four fields — no `provider`/`api_key`/`system_prompt`; the prompt is the module constant `_SYSTEM_PROMPT`.
- **Request:** `/api/chat` via `httpx.AsyncClient(timeout=60)`, `stream=True`, `aiter_lines`. **`keep_alive` is sent on every chat request and on the chat warmup** via `ollama_lifecycle.chat_keep_alive()` (default `"60m"`; `config.llm.keep_alive` override via `getattr` — not a dataclass field; numeric strings such as `"-1"` are sent as numbers), so the model isn't unloaded after Ollama's 5-minute default.
...
- **History:** `query()` clears `_spoken_phrases`/`_reply_finished` at start. After the reply: finished → `_last_response[0]` (full clean text); interrupted → `" ".join(_spoken_phrases)` (a phrase cut mid-way is recorded whole). Then `_conv.add_assistant`, transcript broadcast, `context_manager.record_assistant_turn`. "A cancelled turn with nothing spoken records a short out-of-character marker as the assistant turn instead (see `query()`), so the user's turn is never left with no paired reply at all; a genuinely empty case can no longer occur. `query()` error strings (`ConnectError`/timeout/exception) set `intent["_no_history"]=True` and are spoken but not stored. Cancellation (barge-in) cascades through `asyncio.gather()` into streamer/synth/play tasks.
...
- **Config (`config.tts`):** `voice="af_sky"`, `voice_blend="jf_alpha"`, `blend_ratio=0.92` (→ 8% af_sky / 92% jf_alpha; the "35%" comment in `settings.py` is wrong), `lang_code="a"`, `speed=1`, `output="avatar"`, `device="cpu"` (keeps the GPU free for Ollama). `TTSConfig` has no `cpu_threads` field. `_resolve_tts_device()` returns `config.tts.device` unless `"auto"`/empty (then cuda if available else cpu); `KPipeline(device=...)` is passed only if the installed kokoro accepts it. `_log_kokoro_device`/`_log_cuda_memory` log placement.
...
- **Sleep/wake:** the backend now broadcasts WS state `"sleeping"` once the go-to-sleep goodbye line finishes (`core/speaker.py`'s `Speaker.speak()`), independent of t he socket connection, which stays open throughout. `websocket.js`'s `handleState()` is the single source of truth for the avatar's visual sleep/wake: `"sleeping"` calls `sleepAvatar()`; every other state value calls `wakeAvatar()` (a no-op if already awake) — this also covers the state `ws_server.py` replays to a fresh connection, so a reloaded page ends up correct without guessing from `onopen` alone. `ws.onclose` still calls `sleepAvatar()` directly, since a dropped connection is a signal the backend can't broadcast. Re-wake is idempotent: `startBlinking()` clears `_blinkTimeout`; `startEyeMovement()` runs once per page (`_eyeLoopStarted`).
...
- **State-transition observers** — `core/state.py` gained `add_observer()`/`_notify()` (fired on every genuine transition, best-effort, no sounddevice/ws_server import) so other modules can react to FSM changes without state.py knowing why; `main.py` uses it for the LISTENING watchdog.
- **Shared post-turn reset helper** — `core/turn_lifecycle.py` (new) provides `rest(force_idle=...)`, used by `Processor.handle()` and `llm_service._play_worker`'s `_DONE` branch, checking `state.is_sleeping()` fresh at completion time rather than each site independently deciding whether to force `IDLE`.
---
...
- **`config/settings.py`** singleton `config` (`MayaConfig`): `name="Maya"`, `user_name="senpai"`, `wake_word="wake up Maya"`, `log_level="INFO"`, `log_dir="logs"`, `ws_host="localhost"`, `ws_port=8765`.
  ...
  - `stt`: `STTConfig` now defines only `language="en"` — the previously-listed Whisper-style leftovers (`model_size`/`device`/`compute_type`) have been removed from the dataclass entirely; the transcriber uses Google cloud STT.
...
- **Config surface is now trimmed:** `LLMConfig` has no `provider`/`api_key`/`system_prompt` fields, `TTSConfig` has no `cpu_threads` field, and `STTConfig` has no `model_size`/`device`/`compute_type` fields — these were previously kept as commented-`UNUSED` dataclass fields but have since been removed from `config/settings.py` outright. `api_key` is not needed (Ollama requires none).
...
- `state` holds **one** `_current_task`: anything registering via `run_interruptible` (LLM turn, timer alert, a skill's own `Speaker.speak()` call, main.py's greeting/wake/sleep lines — Batch 4) must not start while another is live — `timer._alert` waits for `not state.is_busy()` for this reason; a second concurrent registration overwrites the first's reference without cancelling it, so callers still rely on the queue's own serialization to avoid this in practice.
- `Processor.handle()` bails out immediately if `state.is_sleeping()` — a queued command must never force PROCESSING and answer after Maya's already been told to sleep; `core/turn_lifecycle.rest()` must be used (not a raw `state.set(IDLE)`) at every turn-completion site so a concurrent sleep is never overwritten.
